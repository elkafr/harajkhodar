...
import 'package:harajkhodar/models/blacklist.dart';
...
class HomeProvider extends ChangeNotifier {
  ...
  Future<String> getUnreadMessage() async {
    final response =
    await _apiProvider.get("https://harajkhodhar.com/api/get_unread_message?user_id=${_currentUser.userId}");
    String messages = '';
    if (response['response'] == '1') {
  ...
  Future<String> getUnreadNotify() async {
    final response =
    await _apiProvider.get("https://harajkhodhar.com/api/get_unread_notify?user_id=${_currentUser.userId}");
    String messages = '';
    if (response['response'] == '1') {
      messages = response['Number'];
    }
  ...
  Future<String> getOmar() async {
    final response =
    await _apiProvider.get("https://harajkhodhar.com/api/social");
    String messages = '';
    if (response['response'] == '1') {
